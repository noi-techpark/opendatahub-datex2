...
// These cover filtering rules that are too date-relative to bake into the
// golden-file equivalence suite (e.g. "started today"), plus a regression
// test for a severe robustness bug found while building the equivalence
// fixtures: the real C# Worker.MapItemsToDatex2 wraps its entire foreach
// loop in one try/catch, so a single item missing the Geo["position"] key
// throws and silently zeroes out the ENTIRE publication cycle, for every
// recipient - not just the one bad item. The Go pipeline must not do that.
