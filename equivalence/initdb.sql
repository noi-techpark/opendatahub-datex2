...
-- Superset of local-test/initdb.sql used to seed the C# app for the
-- equivalence suite: same schema, but covers all 12 subtypes (using the
-- real seed's classnames, bugs included - this is what drives the real,
-- unfixed C# behavior the Go output is compared against), a disabled
-- subtype (prohibition) to prove disabled-subtype handling matches, and a
-- second recipient to prove per-recipient fan-out.
...
2	Test secondo destinatario	t	f	ITPBZ	IT999	DatexOrig	1,2,3,4	t	\N
\.
...
8	Traffico proibito	GENERIC	traffic-event:prohibition	RoadOrCarriagewayOrLaneManagement	other	\N	\N	f	f	f	high
